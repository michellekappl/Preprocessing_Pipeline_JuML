...
function stop_word_removal(input::Vector{String}; language::String="English", custom_stopwords::Union{Nothing, Set{String}}=nothing)
    @eval using StopWords
    #use default stopword list or use customer provided one
    stopwords_set = custom_stopwords === nothing ? stopwords[language] : custom_stopwords
